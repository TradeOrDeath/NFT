...
//near_contract_standards::impl_non_fungible_token_core!(MyNFTMarket, tokens);
near_contract_standards::impl_non_fungible_token_approval!(MyNFTMarket, tokens);
near_contract_standards::impl_non_fungible_token_enumeration!(MyNFTMarket, tokens);
...
impl MyNFTMarket {
    ...
    pub fn get_nft_token(&self, token_id: TokenId) -> Option<Token> {
    ...
    pub fn place_bid(&mut self, auction_id: u128) {
        let mut auction = self.auction_by_id.get(&auction_id).expect("Auction does not exist");
        assert_eq!(
        ...
        assert_eq!(
            ...
            true,
            "Bid price must be greater than the current winning bid"
        );
        if !auction.winner.is_empty() {
            let old_winner = Promise::new(auction.winner.clone());
    ...
    pub fn claim_near(&mut self, auction_id: u128) {
        ...
        assert_eq!(
            env::predecessor_account_id(),
            auction.owner,
            "You are not the operator of this auction"
        );
        assert_eq!(
            env::block_timestamp() > auction.end_time,
            true,
        ...
        assert_eq!(
            auction.is_near_claimed,
            false,
            "You have already claimed NEAR tokens"
        );
        Promise::new(auction.owner.clone()).transfer(auction.current_price);
    ...
    pub fn get_auction(&self, auction_id: u128) -> Auction {
        self.auction_by_id.get(&auction_id).expect("Auction does not exist")
    }
}
